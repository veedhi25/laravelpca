...
const fetchCategories = async ({ queryKey }: QueryParamsType) => {
  ...
  const url = `${API_ENDPOINTS.FETCH_SHOP_CATEGORIES}?search=${searchString}&searchJoin=and&limit=${limit}&parent=${parent}&page=${page}&orderBy=${orderBy}&sortedBy=${sortedBy}`;
  const {
    data: { data, ...rest },
...
const useParentCategoriesQuery = (options: CategoriesQueryOptionsType) => {
  return useQuery<any, Error>(
    [API_ENDPOINTS.FETCH_SHOP_CATEGORIES, options],
    fetchCategories,
    {
