...
import ImportProducts from "./import-master-products";
import ImportVariationOptions from "./import-all-variation-options";
  ...
  return (
    <Card className="flex flex-col min-h-screen md:min-h-0">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        ...
        <a
          href={`${process?.env?.NEXT_PUBLIC_REST_API_ENDPOINT}/export-master-products`}
          target="_blank"
        ...
        <a
          href={`${process?.env?.NEXT_PUBLIC_REST_API_ENDPOINT}/export-master-variation-options`}
          target="_blank"
          className="border-dashed border-2 border-border-base h-36 rounded flex flex-col justify-center items-center cursor-pointer focus:border-accent-400 focus:outline-none p-5"
        >
          <DownloadIcon className="text-muted-light w-10" />
