...
const BatchesList = ({ batches, onPagination }: IProps) => {
    
  const { t } = useTranslation();
  const { data, paginatorInfo } = batches!;
  console.log('class',batches);
//   return ''
...
//   const filteredClasses = data?.filter((cat)=> cat?.parent == null)
  ...
  const columns = [
    ...
    
    {
        ...
        width: 150,
        render: (text, record) => {  
            return <span className="">{record?.student_class?.name}th</span>;  
        }
    }
